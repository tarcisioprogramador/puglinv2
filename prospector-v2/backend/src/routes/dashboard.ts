...
import { getDb, SQL_DAYS_SINCE } from '../database';
...
router.get('/stats', async (_req: Request, res: Response) => {
  try {
    ...
    const stats = {
      ...
      followupsPendentes: (await db.prepare(`SELECT COUNT(*) as c FROM leads WHERE status='proposta' AND dataProposta IS NOT NULL AND ${daysExpr} >= 4`).get() as any).c,
